...
export default function Profesionales() {
  return (
    <>
      <Head><title>Apoyo profesional — AIvora</title></Head>
      <div className="phone-bg">
        <div className="phone">
          ...
          <div style={{padding:'4px 22px 14px',position:'relative'}}>
            ...
            <p style={{fontSize:11.5,color:'var(--gray)'}}>Profesionales verificados</p>
            <h2 style={{fontFamily:'var(--f-serif)',fontSize:24,fontWeight:700,color:'var(--coal)'}}>Apoyo cerca de ti</h2>
          </div>

          <div className="scroll" style={{paddingBottom:80}}>
            <div className="filters">
              {['Todos','Disponibles','Psicólogos','Psiquiatras', "General"].map((f,i)=>(
                <button key={f} className={`fchip ${i===0?'on':'off'}`}>{f}</button>
              ))}
            </div>

            {PROFESSIONALS.map((p,i)=>(
              // All professionals redirect to /profesionales/1 (same detail for all)
              <Link key={p.id} href="/usuario/profesionales/1" className={`au${Math.min(i+1,4)}`} style={{
                display:'flex',background:'var(--white)',borderRadius:20,padding:'15px 15px',
                marginBottom:10,boxShadow:'0 3px 12px rgba(0,0,0,.06)',
