...
export default function ProDashboard() {
  const stats = [
    {label:'Pacientes activos',val:'12',delta:'+2 esta semana',color:'var(--sage)'},
    {label:'Pendientes',val:'3',delta:'2 urgentes',color:'var(--red)'},
    {label:'Sesiones mes',val:'28',delta:'+15% vs anterior',color:'var(--blue)'},
    {label:'Calificación',val:'4.9★',delta:'156 reseñas',color:'var(--amber)'},
  ];

  return (
    <>
      <Head><title>Dashboard — AIvora Pro</title></Head>
      <div className="phone-bg">
        <div className="phone phone-sage">
          ...
          <div style={{padding:'4px 22px 14px',position:'relative'}}>
            ...
            <h2 style={{fontFamily:'var(--f-serif)',fontSize:23,fontWeight:700,color:'var(--coal)'}}>Hola, Dra. Huanca 👋</h2>
            <span className="badge" style={{background:'rgba(90,143,107,.14)',color:'var(--sage-d)',marginTop:4,display:'inline-flex'}}>PSICÓLOGA VERIFICADA ✓</span>
          </div>
          ...
          <div className="scroll" style={{paddingBottom:80}}>
            ...
            <div className="au2" style={{background:'rgba(200,75,75,.08)',borderRadius:18,padding:14,marginBottom:14,border:'1.5px solid rgba(200,75,75,.18)'}}>
              <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:10}}>
                ...
                <h3 style={{fontFamily:'var(--f-serif)',fontSize:15,color:'var(--coal)'}}>Requieren atención pronto</h3>
              </div>
              {PATIENTS.filter(p=>p.nivel==='Requiere atención').map((p,i)=>(
                <Link key={i} href="/profesional/pacientes/1" style={{display:'flex',gap:11,alignItems:'center',background:'var(--white)',borderRadius:13,padding:'12px 13px',textDecoration:'none',boxShadow:'0 2px 8px rgba(0,0,0,.06)',marginBottom:i<1?8:0}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <p style={{fontSize:11,color:'var(--gray)'}}>{p.date} · {p.age} años · {p.region}</p>
                  </div>
            ...
            {/* Solicitudes */}
            <h3 style={{fontFamily:'var(--f-serif)',fontSize:16,fontWeight:600,color:'var(--coal)',marginBottom:12}}>Solicitudes nuevas</h3>
            {[{name:'Pedro V.',region:'Cajamarca',time:'hace 22 min'},{name:'Rosa F.',region:'Cajamarca',time:'hace 1h'}].map((s,i)=>(
              <div key={i} className={`au${i+3}`} style={{background:'var(--white)',borderRadius:15,padding:'12px 14px',marginBottom:8,display:'flex',gap:11,alignItems:'center',boxShadow:'0 2px 8px rgba(0,0,0,.05)'}}>
                ...
                <div style={{flex:1}}>
                  <p style={{fontSize:13,fontWeight:700,color:'var(--coal)'}}>{s.name}</p>
                  <p style={{fontSize:11,color:'var(--gray)'}}>{s.region} · {s.time}</p>
                </div>
                <button style={{padding:'7px 13px',borderRadius:19,border:'none',fontSize:12,fontWeight:700,background:'rgba(90,143,107,.18)',color:'var(--sage-d)',cursor:'pointer',fontFamily:'var(--f-sans)'}}>Aceptar</button>
              </div>
            ))}
          </div>
